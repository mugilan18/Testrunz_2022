...
class FirstYChemistry:
    ...
    def Colorimetry(self):
        ...
        lst=[float(argument[2]),float(argument[4]),float(argument[6]),float(argument[8]),float(argument[10])]
        K=float(argument[11])
        nearvalue=closest(lst,K)
        dict = {float(argument[1]):float(argument[2]),float(argument[3]):float(argument[4]),float(argument[5]):float(argument[6]),float(argument[7]):float(argument[8]),float(argument[9]):float(argument[10])} 
        ans = get_key(nearvalue)
        print(json.dumps({"Amount":[{"The  Iron in given solution" : ans }]}))
